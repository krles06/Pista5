...
export default function MesocicloNode({ mesociclo, macrociclo }: Props) {
    ...
    return (
        <Card className="border-emerald-800/30 bg-card/80 shadow-sm overflow-hidden transition-all">
            <CardHeader className="p-0">
                ...
                >
                    <div className="flex items-center gap-2">
                        ...
                        <div>
                            <CardTitle className="text-base text-foreground flex items-center gap-2">
                                {mesociclo.nombre}
                                {mesociclo.tipo && <span className="text-[10px] font-normal px-2 py-0.5 rounded border bg-muted border-border text-muted-foreground">{mesociclo.tipo}</span>}
                            </CardTitle>
                    ...
                    <div className="flex items-center" onClick={(e) => e.stopPropagation()}>
                        <Button variant="ghost" size="icon" className="h-7 w-7 text-muted-foreground hover:text-emerald-400" onClick={() => setIsAddMicroModalOpen(true)}>
                            <Plus className="h-3 w-3" />
                        </Button>
                        <Button variant="ghost" size="icon" className="h-7 w-7 text-muted-foreground hover:text-foreground" onClick={() => setIsEditModalOpen(true)}>
                            <Edit2 className="h-3 w-3" />
                        </Button>
                        <Button variant="ghost" size="icon" className="h-7 w-7 text-muted-foreground hover:text-red-400" onClick={() => setIsDeleteAlertOpen(true)}>
            ...
            {isExpanded && (
                <CardContent className="p-0 border-t border-border/20 bg-background/50">
                    <div className="p-3 pl-8 space-y-2 relative before:absolute before:inset-y-0 before:left-4 before:w-[px] before:bg-muted/50">
                        {isLoading ? (
                            <div className="text-xs text-muted-foreground py-1">Cargando microciclos...</div>
                        ) : !microciclos || microciclos.length === 0 ? (
                            <div className="text-xs text-muted-foreground py-2 border border-dashed border-border rounded px-3">
                                No hay microciclos.
