            ...
            <div className="flex flex-col sm:flex-row justify-between gap-4">
                ...
                <div className="flex items-center gap-2 bg-card border border-border p-1 rounded-lg">
                    <Button
                        variant="ghost"
                        size="sm"
                        className={`px-3 py-1.5 h-auto ${viewMode === 'grid' ? 'bg-muted text-foreground' : 'text-muted-foreground hover:text-foreground'}`}
                    ...
                    <Button
                        variant="ghost"
                        size="sm"
                        className={`px-3 py-1.5 h-auto ${viewMode === 'list' ? 'bg-muted text-foreground' : 'text-muted-foreground hover:text-foreground'}`}
            ...
            {isLoading ? (
                ...
                <Card className="border-border bg-card shadow-xl border-dashed">
                    <CardContent className="flex flex-col items-center justify-center py-16 text-center">
                        ...
                        <h3 className="text-xl font-medium text-foreground">No hay jugadores</h3>
                                ...
                                >
                                    <CardHeader className="p-5 flex flex-row items-center gap-4 space-y-0">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <p className="text-xs text-muted-foreground mt-0.5 truncate uppercase tracking-wider font-medium">
                                                {j.posicion || 'Sin posición'}
                                            </p>
                                        </div>
                                        <Badge className="bg-background border-border text-muted-foreground font-bold h-7 min-w-[28px] justify-center">
                                            {j.dorsal || '--'}
                                        </Badge>
                                    </CardHeader>
                                    <CardContent className="p-5 pt-0 flex-1 flex flex-col justify-between gap-4">
                                        ...
                                        <div className="flex justify-between items-center pt-2 border-t border-border">
                                            <div className="flex gap-2">
                                                {j.telefono && <Phone className="h-3 w-3 text-muted-foreground" />}
                                                {j.email && <Mail className="h-3 w-3 text-muted-foreground" />}
                                            </div>
                                            <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                                <Button variant="ghost" size="icon" className="h-7 w-7 text-muted-foreground hover:text-foreground" onClick={(e) => { e.stopPropagation(); navigate(`/jugadores/${j.id}/editar`); }}>
                                                    <Edit className="h-3.5 w-3.5" />
                                                </Button>
                                                <Button variant="ghost" size="icon" className="h-7 w-7 text-muted-foreground hover:text-red-400" onClick={(e) => { e.stopPropagation(); setDeleteId(j.id); }}>
                        ...
                        <div className="rounded-xl border border-border bg-card overflow-hidden">
                            <table className="w-full text-sm text-left">
                                ...
                                <tbody className="divide-y divide-zinc-800/50">
                                    {filteredJugadores.map((j) => (
                                        <tr key={j.id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={() => navigate(`/jugadores/${j.id}`)}>
                                            <td className="px-6 py-4">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div className="font-medium text-foreground">{j.nombre} {j.apellidos}</div>
                                                </div>
                                            </td>
                                            <td className="px-6 py-4 text-muted-foreground">{j.posicion || '--'}</td>
                                            ...
                                            <td className="px-6 py-4 text-right" onClick={(e) => e.stopPropagation()}>
                                                <div className="flex justify-end gap-2">
                                                    <Button variant="ghost" size="icon" className="h-8 w-8 text-muted-foreground hover:text-foreground" onClick={() => navigate(`/jugadores/${j.id}/editar`)}>
                                                        <Edit className="h-4 w-4" />
                                                    </Button>
                                                    <Button variant="ghost" size="icon" className="h-8 w-8 text-muted-foreground hover:text-red-400" onClick={() => setDeleteId(j.id)}>
